...
#urls와 view가 다른 파일 위치이므로 절대경로로 표현
from vote.views import * #vote폴더 안에 views파일 중 index함수 추가
urlpatterns = [
    path('admin/', admin.site.urls),
    #path(문자열, 함수명) : 웹 클라이언트가 문자열에 해당하는 URL로 접근시 매칭된 함수를 호출함
    #http://127.0.0.1:8000 주소로 접근시 vote 어플리케이션 내의 index 함수 호출
    #http://127.0.0.1:8000/vote/ 로 시작하는 URL들을 vote폴더 안에있는 urls 파이썬파일로 처리하도록 매칭

...
#reverse() : 해당 함수를 호출했을 때 바로 즉시 urls 파일을 탐색해 url주소를 찾음
#reverse_lazy() : 해당 함수를 호출했을 때 웹서버 실행이 준비되면 urls파일을 탐색해 url주소를 찾음
LOGOUT_REDIRECT_URL = reverse_lazy('vote:index')
#로그인이 요구되는 기능을 비로그인 상태로 접근할때 로그인할수 있는 페이지 설정
...
#클라이언트 id
SOCIAL_AUTH_GOOGLE_OAUTH2_KEY = '559456206143-p069p61t8mbq7jtb7l8saqv68tanq4nq.apps.googleusercontent.com'
#클라이언트 보안비밀
SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET = 'UJNwWPFEtnjFLkMzT0WNgngw'
...
TEMPLATES = [
    {
        ...
        'DIRS': [BASE_DIR],#어느 폴더에 HTML파일을 저장할지 설정
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [

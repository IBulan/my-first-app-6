...
#reverse(문자열, args=튜플)
#문자열에 해당하는 url 별칭을 찾고, 매개변수가 필요한 URL일 경우 args 매개변수에 있는 튜플값으로 자동 매핑
...
def index(request):
    print("index 함수 호출")
    #Question.objects.all : Question 모델 클래스에 저장된 모든 객체 추출
...
def vote(request,question_id):
    ...
    if request.method == "POST":
        ...
        #request.POST.get(문자열) : POST방식으로 들어온 데이터 중 name 속성의 값이 문자열과 같은 데이터를 추출
...
def registerQ(request):
    if request.method == "GET":
        form = QuestionForm() #Question 객체 생성, 사용하는 속성들이 공란으로 되어있음
        ...
        if form.is_valid():
            #폼객체.save() : 해당 폼에 입력값들로 모델클래스 객체를 데이터 베이스에 저장후 반환
            #폼객체.save(commit=False) : 데이터베이스에 바로 저장하지 않고 모델폼에 모델클래스 객체로 변환 후 반환
...
def deleteQ(request, question_id):
    ...
    if request.user.get_username() != obj.author.username:
        return render (request,"vote/templates/error.html",{'error':"잘못된 접근 입니다.",
...
def deleteC(request, choice_id):
    ...
    if request.user.get_username() != obj.question.author.username:
        return render (request,"vote/templates/error.html",{'error':"잘못된 접근 입니다.",
                      'returnURL':reverse('vote:detail',args=(obj.question.id,))})
    id = obj.question.id
    obj.delete()
...
def registerC(request,question_id):
    obj = get_object_or_404(Question, pk = question_id)
    if request.user.get_username() != obj.author.username:
        return render (request,"vote/templates/error.html",{'error':"본인이 작성한 글이 아닙니다.",
                      'returnURL':reverse('vote:detail',args=(question_id,))})
    if request.method == "GET":
        #폼 객체 생성
        form = ChoiceForm()
        ...
        if form.is_valid():
            ...
            obj2.question = obj
            obj2.save()
            return HttpResponseRedirect(reverse('vote:detail', args=(obj2.question.id,) ) )
        else:

...
class Post(models.Model):
    ...
    tpye = models.ForeignKey(PostType, on_delete=models.CASCADE)
    ...
    pub_data = models.DateField('날짜', auto_now_add=True)
...
class Image(models.Model):
    ...
    #ImageField는 이미지가 저장된 경로를 가지고있어, 객체가 제거되도 실제 이미지 파일은 제거되지 않음. 때문에 delete 함수를 오버라이딩해 해당경로에 저장된 이미지를 지우는 행동이 필요함
